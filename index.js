...
QRCodeModel.prototype = {
    ...
    setupPositionProbePattern: function (row, col) {
        for (let r = -1; r <= 7; r++) {
            ...
            for (let c = -1; c <= 7; c++) {
                ...
                r >= 0 && r <= 6 && (c == 0 || c == 6) || c >= 0 && c <= 6 && (r == 0 || r == 6) || r >= 2 && r <= 4 && c >= 2 && c <= 4 ? this.modules[row + r][col + c] = !0 : this.modules[row + r][col + c] = !1;
            }
        }
    },
    ...
    createMovieClip: function (target_mc, instance_name, depth) {
        const qr_mc = target_mc.createEmptyMovieClip(instance_name, depth), cs = 1;
        this.make();
        for (let row = 0; row < this.modules.length; row++) {
            const y = row * cs;
            for (let col = 0; col < this.modules[row].length; col++) {
                const x = col * cs, dark = this.modules[row][col];
    ...
    setupPositionAdjustPattern: function () {
        ...
        for (let i = 0; i < pos.length; i++) {
            for (let j = 0; j < pos.length; j++) {
                ...
                for (let r = -2; r <= 2; r++) for (let c = -2; c <= 2; c++) r == -2 || r == 2 || c == -2 || c == 2 || r == 0 && c == 0 ? this.modules[row + r][col + c] = !0 : this.modules[row + r][col + c] = !1;
            }
        }
    },
...
QRCodeModel.createData = function (typeNumber, errorCorrectLevel, dataList) {
    const rsBlocks = QRRSBlock.getRSBlocks(typeNumber, errorCorrectLevel), buffer = new QRBitBuffer();
...
QRCodeModel.createBytes = function (buffer, rsBlocks) {
    ...
    for (let r = 0; r < rsBlocks.length; r++) {
        ...
        offset += dcCount;
        const rsPoly = QRUtil.getErrorCorrectPolynomial(ecCount), rawPoly = new QRPolynomial(dcdata[r], rsPoly.getLength() - 1), modPoly = rawPoly.mod(rsPoly);
    ...
    for (let i = 0; i < maxDcCount; i++) for (let r = 0; r < rsBlocks.length; r++) i < dcdata[r].length && (data[index++] = dcdata[r][i]);
    for (let i = 0; i < maxEcCount; i++) for (let r = 0; r < rsBlocks.length; r++) i < ecdata[r].length && (data[index++] = ecdata[r][i]);
    return data;
};
...
const QRMode = {
        ...
        getLostPoint: function (qrCode) {
            ...
            for (let row = 0; row < moduleCount; row++) for (let col = 0; col < moduleCount - 6; col++) qrCode.isDark(row, col) && !qrCode.isDark(row, col + 1) && qrCode.isDark(row, col + 2) && qrCode.isDark(row, col + 3) && qrCode.isDark(row, col + 4) && !qrCode.isDark(row, col + 5) && qrCode.isDark(row, col + 6) && (lostPoint += 40);
            for (let col = 0; col < moduleCount; col++) for (let row = 0; row < moduleCount - 6; row++) qrCode.isDark(row, col) && !qrCode.isDark(row + 1, col) && qrCode.isDark(row + 2, col) && qrCode.isDark(row + 3, col) && qrCode.isDark(row + 4, col) && !qrCode.isDark(row + 5, col) && qrCode.isDark(row + 6, col) && (lostPoint += 40);
            let darkCount = 0;
            for (let col = 0; col < moduleCount; col++) for (let row = 0; row < moduleCount; row++) qrCode.isDark(row, col) && darkCount++;
...
QRPolynomial.prototype = {
    ...
    mod: function (e) {
        ...
        const ratio = QRMath.glog(this.get(0)) - QRMath.glog(e.get(0)), num = new Array(this.getLength());
...
class QRCode extends React.Component{
    ...
    componentWillReceiveProps (nextProps) {
        ...
        const newProps = _.pick(nextProps, keys);
        if (!_.isEqual(oldProps, newProps)) {
            const { text, correctLevel } = newProps;
            this.oQRCode = new QRCodeModel(_getTypeNumber(text, correctLevel), correctLevel);
            this.oQRCode.addData(text);
            this.oQRCode.make();
        }
    ...
    renderRow (oQRCode, row, width, nCount, nWidth, nHeight, colorDark, colorLight) {
        ...
        for (let col = 0; col < nCount; col++) {
            items.push(this.renderCell(oQRCode, row, col, nWidth, nHeight, colorDark, colorLight));
        }
        return (
            <View style={{ flexDirection:'row', height:nHeight, width:width }} key={row}>
                {items}
            </View>
        );
    }
    ...
    render () {
        ...
        for (let row = 0; row < nCount; row++) {
            rows.push(this.renderRow(oQRCode, row, width, nCount, nWidth, nHeight, colorDark, colorLight));
        }
        return (
